...
// Убедитесь, что у вас есть настроенные учетные данные
try {
  // Используем явные учетные данные из файла
  admin.initializeApp();
  console.log('Firebase Admin SDK успешно инициализирован');
} catch (error) {
  console.error('Ошибка инициализации Firebase Admin SDK:', error);
  console.log('!!! ВАЖНО: Установите переменную окружения GOOGLE_APPLICATION_CREDENTIALS !!!');
  console.log('Пожалуйста, выполните в командной строке:');
  console.log('set GOOGLE_APPLICATION_CREDENTIALS="путь\\к\\вашему\\файлу\\учетных\\данных.json"');
  console.log('Затем запустите импорт заново');
  throw error;
}
...
async function startImport() {
  ...
  if (args.length < 3) {
    console.log('Использование: npm run import-xlsx <путь_к_файлу> <название_листа> <тип_данных>');
  ...
  const dataType = args[2].toLowerCase();
